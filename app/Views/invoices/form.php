...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white border-0 pt-4 px-4 d-flex justify-content-between align-items-center">
        <div>
            <h3 class="h5 mb-1">Nouvelle facture</h3>
            <p class="text-muted mb-0">Créer une facture mixte produits / services.</p>
        </div>
        <a href="<?= e(url('/invoices')); ?>" class="btn btn-outline-secondary">Retour</a>
    </div>
    <div class="card-body px-4 pb-4">
        <form method="post" action="<?= e($formAction); ?>" class="row g-3" id="invoiceForm">
            <?= csrf_field(); ?>
            <div class="col-md-4">
            ...
            <div class="col-md-4">
                <label class="form-label" for="invoice_date">Date facture</label>
                <input type="date" class="form-control" id="invoice_date" name="invoice_date" value="<?= e(old('invoice_date', date('Y-m-d'))); ?>" required>
            </div>
            <div class="col-md-4">
            ...
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <label class="form-label mb-0">Lignes de facture</label>
                    <button type="button" class="btn btn-sm btn-outline-primary" id="addInvoiceRow">Ajouter une ligne</button>
                </div>
                <div class="table-responsive">
                    <table class="table align-middle" id="invoiceItemsTable">
                        <thead><tr><th>Type</th><th>Référence</th><th>Description</th><th>Qté</th><th>Prix</th><th>Total</th><th></th></tr></thead>
                        <tbody>
                            <?php foreach ($oldItems as $item): ?>
                                <tr>
                                    <?php $itemType = $item['item_type'] ?? 'product'; ?>
                                    <td><select class="form-select item-type" name="items[item_type][]"><option value="product" <?= $itemType === 'product' ? 'selected' : ''; ?>>Produit</option><option value="service" <?= $itemType === 'service' ? 'selected' : ''; ?>>Service</option></select></td>
                                    <td>
                                        <select class="form-select product-select <?= $itemType === 'service' ? 'd-none' : ''; ?>" name="items[product_id][]"><option value="">Produit</option><?php foreach ($products as $product): ?><option value="<?= e((string) $product['id']); ?>" data-name="<?= e($product['name']); ?>" data-price="<?= e((string) $product['sale_price']); ?>" <?= (int) ($item['product_id'] ?? 0) === (int) $product['id'] ? 'selected' : ''; ?>><?= e($product['name'] . ' (' . $product['sku'] . ')'); ?></option><?php endforeach; ?></select>
                                        <select class="form-select service-select <?= $itemType === 'product' ? 'd-none' : ''; ?>" name="items[service_id][]"><option value="">Service</option><?php foreach ($services as $service): ?><option value="<?= e((string) $service['id']); ?>" data-name="<?= e($service['name']); ?>" data-price="<?= e((string) $service['unit_price']); ?>" <?= (int) ($item['service_id'] ?? 0) === (int) $service['id'] ? 'selected' : ''; ?>><?= e($service['name'] . ' (' . $service['code'] . ')'); ?></option><?php endforeach; ?></select>
                                    </td>
